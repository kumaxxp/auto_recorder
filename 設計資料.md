...
## 🚀 **Phase D: WebSocket + Canvas ストリーミング (2025/11/24)**

- **FramePublisher** (`camera_stream.py`): UIワーカーから渡された BGR フレームを `np.ascontiguousarray(frame).tobytes()` で保持し、`/stream` WebSocket 経由で最新1枚のみを push。ヘッダーは `width/height/channels` の 12byte 固定。
- **NiceGUI 拡張** (`main.py`, `ui.py`): `@app.websocket("/stream")` でサーバー側ハンドラを追加し、ダッシュボードには `<canvas id="stream-...">` を配置。`ui.add_head_html` で注入した JS が `requestAnimationFrame` ループ内で `ArrayBuffer` を `ImageData` に復号し、<canvas>へ描画。
- **既存 UI との共存**: 透過 `ui.interactive_image` を canvas 上に重ね、従来のセグメントクリック／右ドラッグROI操作を維持。ROI位置は slider＋「Reset ROI auto」ボタンでも変更可能。
- **高速化ポイント**: RGBA→BGR の合成を整数演算 (`((base*(255-alpha)+mask*alpha)//255)`) へ刷新し、`DISPLAY_TARGET_FPS=40` 以内でストリームを生成。JPEG/Base64 を排除したことで UI 側の I/O は WebSocket のみ。

### 📈 実測 ( `./.venv/bin/python main.py --duration 20 --no-browser` )
| 指標 | 平均値 | 備考 |
| --- | --- | --- |
| `PhaseC worker` | 36〜39 FPS | DeepStream + Cityscapes 推論スレッド。アルファ合成後も 35FPS 以上を維持。 |
| `PhaseD display` | 32〜35 FPS | WebSocket canvas での描画速度。ブラウザ1タブで安定 33FPS 前後。 |

### 🔧 検証手順
1. `./.venv/bin/python main.py --no-browser` を起動。
2. 既存ブラウザで `http://<jetson-ip>:8080` を開き、canvas が自動再生されることを確認。
3. ROI スライダー／右ドラッグ操作でライン位置を変更すると、`Status` と `steer` 表示が即座に更新される。
4. ブラウザ開発者ツールで `Frames/sec` を計測すると 32〜35FPS 程度で安定し、Cityscapes overlay も保持されている。

### 📌 メモ
- WebSocketクライアントはリロード時に自動再接続する。複数ブラウザを開いた場合も最新フレームのみ共有されるため、サーバー負荷は一定。
- さらなるFPS向上（40FPS超）には Phase5 で予定している MJPEG/WebRTC 化、または DeepStream 側でのハードウェアブレンド出力が有効。

## 🔁 **Phase D2: デュアルカメラ Canvas ストリーミング (2025/11/24)**

### 🧩 実装ポイント
- **MultiFramePublisher** (`camera_stream.py`): カメラ名ごとに `FramePublisher` を動的生成し、`/stream/{stream_name}` WebSocket に多重配信。Front / Bottom が同じ API を共有できる。
- **FastAPI ルート** (`main.py`): `@app.websocket("/stream/{stream_name}")` で任意のストリームを購読。NiceGUI 側は `startStreamCanvas('...', '/stream/bottom')` のように接続先を切り替えるだけでよい。
- **SegmentationDashboard 拡張** (`ui.py`): 1つのダッシュボード内で Bottom 表示＋制御、Front 表示専用キャンバスをまとめて描画。デュアル構成時はヘッダーに「デュアルカメラビュー」とレイアウトトグル（縦／横）を表示。
- **ROI 操作の限定**: 透過オーバーレイと ROI ラインは Bottom (足元) カメラに限定し、Front では GPU→CPU 転送のみ行うことで負荷を抑制。
- **バックグラウンド動作**: `dashboard1.start_processing_only()` を導入し、UIにマウントしない Front カメラもワーカー＆WebSocket だけを走らせられる。

### 🖥️ UI 挙動
- **レイアウトトグル**: `stack` (縦) / `side` (横) を NiceGUI の `ui.toggle` で即時切替。Tailwind の `flex-direction` を差し替えるだけなのでリロード無しでレイアウトを反映できる。
- **自動接続**: どちらの canvas も `requestAnimationFrame` ベースで描画し、再接続ロジックは Phase D を踏襲。複数タブを開いても最新フレームだけが共有され帯域が安定。
- **ラベル**: Bottom 側は既存のスライダ／セグメントボタン／ステータスを保持。Front 側には「表示専用 (ROI操作なし)」ラベルを付けて誤操作を防止。

### ⚙️ 起動手順
1. `main.py` の `ENABLE_DUAL_CAMERA = True` を設定し、前方カメラ (`index=1`) を有効化。
2. `./.venv/bin/python main.py --no-browser` で NiceGUI を起動。
3. ブラウザで `http://<host>:8080` にアクセスすると、Bottom/Front の2枚が同ページに並ぶ。Front のみを別ページに出す場合は今後 `/front` などを追加予定。

### 📊 現状計測 (`./.venv/bin/python main.py --duration 15 --no-browser`)
| 指標 | 平均値 (概算) | メモ |
| --- | --- | --- |
| `PhaseC worker` | 30〜33 FPS | Cityscapes + DeepStream スレッド。`MultiFramePublisher` 追加後もほぼ同等に推移。 |
| `PhaseD display (bottom)` | 30〜33 FPS | WebSocket canvas。瞬間的に 22〜25FPS まで落ちるが数フレーム以内に復帰。 |

> Front カメラは実機側で未接続のため、現状は UI キャンバスのみ確認済み。センサー配線後に `PhaseD display (front)` の FPS を採取予定（目標 ≥25 FPS）。

### 📌 今後のTODO
1. Front カメラを実配線し、デュアル状態での CPU/GPU 負荷と FPS を採取。
2. Front キャンバスにも簡易指標（例: ヨー角や直近障害物距離）を重ね、操縦者が状況判断しやすくする。
3. 2面配置時のレイアウトを Tailwind のブレークポイントに合わせて最適化し、タブレットでもスライダ領域が崩れないようにする。



## 🧩 **Phase F: Floor/Wall 幾何分離 + 3段階融合 (2025/11/24)**

### 目的
Cityscapes のクラスだけでは road/sidewalk と wall/building の境界が曖昧なケースがあるため、ボトムカメラのフレーム勾配から「水平床」と「垂直壁」を推定し、Fusion ロジックに組み込む。

### 技術アプローチ
1. **ROI 限定グレースケール**: Bottom カメラの最新フレームを取得し、ROI 以降（`roi_start = dashboard.roi_y`）を `cv2.cvtColor(..., COLOR_BGR2GRAY)` した領域だけ処理。
2. **Sobel/Scharr 勾配**: `cv2.Sobel(roi, CV_32F, 1, 0)` / `cv2.Sobel(roi, CV_32F, 0, 1)` で gx, gy を計算し、`mag = sqrt(gx^2 + gy^2)` を 0–1 に正規化。必要に応じて Scharr (`cv2.Scharr`) に切り替え可能。
3. **幾何マスク生成**:
    - `floor_geometry_mask = mag < T_floor` （例: 0.08）で緩やかな勾配＝水平面を抽出。
    - `wall_geometry_mask = mag > T_wall` （例: 0.18）で急勾配＝垂直面を抽出。
4. **信頼度算出**: `geometry_confidence = floor_pixels / roi_pixels` と `wall_ratio` を算出し、FusionMonitor と UI に渡す。`floor/wall separation active` のフラグも状態化。
5. **セグメンテーションとの融合**: `SegmentationProcessor.process_bottom_mask` が返す drivable 判定を `final_drivable = drivable_mask & floor_geometry_mask` で絞り込み、床信頼度が低い場合は自動で `AVOID/STOP` にダウングレード。
6. **FusionMonitor 拡張**: 既存ロジックに幾何情報を追加し、
    - `front.detected -> STOP`
    - `geometry_confidence < 0.45 -> AVOID`
    - それ以外は bottom decision に従う。
7. **UI 表示**: Fusion Telemetry に `geometry=0.82` や `floor/wall separation active` を追加、信頼度に応じて色分け（>=0.7: 緑 / 0.4–0.7: 橙 / <0.4: 赤）。
8. **性能考慮**: ROI 下部のみ処理＆ 8bit→32F 変換最小化により、追加負荷は 1–2ms 程度。更新頻度は FusionMonitor と同じ 50ms を想定し、必要ならサブサンプリングする。

### 受け入れ基準
- 30FPS 以上を維持。
- 壁が Cityscapes で road と誤判定された場合でも、幾何マスクで `final_drivable` から除外される。
- UI 側に `geometry` 値と分離ステータスが明示される。
